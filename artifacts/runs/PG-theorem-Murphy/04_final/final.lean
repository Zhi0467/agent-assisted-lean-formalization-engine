...
noncomputable def dotProductCLM : (ι → ℝ) →L[ℝ] (ι → ℝ) →L[ℝ] ℝ :=
  LinearMap.toContinuousLinearMap
    (((LinearMap.toContinuousLinearMap : ((ι → ℝ) →ₗ[ℝ] ℝ) ≃ₗ[ℝ] ((ι → ℝ) →L[ℝ] ℝ)).toLinearMap).comp
      (dotProductBilin ℝ ℝ))

noncomputable def vecMulCLM : (ι → ℝ) →L[ℝ] Matrix ι ι ℝ →L[ℝ] (ι → ℝ) :=
  LinearMap.toContinuousLinearMap
    (((LinearMap.toContinuousLinearMap : (Matrix ι ι ℝ →ₗ[ℝ] (ι → ℝ)) ≃ₗ[ℝ]
...
lemma rowBellman {p0 d : ι → ℝ} {A : Matrix ι ι ℝ}
  ...
  have hshift_map : HasSum (fun k : ℕ => Matrix.vecMul (f k) A) (Matrix.vecMul d A) := by
    simpa [f] using hd.mapL (LinearMap.toContinuousLinearMap (Matrix.vecMulLinear A))
  ...
  calc
    ...
    _ = Matrix.vecMul d (1 - A) := by simp [Matrix.vecMul_sub]
...
lemma colBellman {q r : ι → ℝ} {A : Matrix ι ι ℝ}
  ...
  have hshift_map : HasSum (fun k : ℕ => Matrix.mulVec A (f k)) (Matrix.mulVec A q) := by
    simpa [f] using hq.mapL (LinearMap.toContinuousLinearMap (Matrix.mulVecLin A))
  ...
  calc
    ...
    _ = Matrix.mulVec (1 - A) q := by simp [Matrix.sub_mulVec]

lemma jEqDot {p0 d r : ι → ℝ} {A : Matrix ι ι ℝ} {J : ℝ}
    (hd : HasSum (fun k : ℕ => Matrix.vecMul p0 (A ^ k)) d)
    (hJ : HasSum (fun k : ℕ => dotProduct (Matrix.vecMul p0 (A ^ k)) r) J) :
    J = dotProduct d r := by
  have hd_map : HasSum (fun k : ℕ => dotProduct r (Matrix.vecMul p0 (A ^ k))) (dotProduct r d) := by
    simpa using hd.mapL (LinearMap.toContinuousLinearMap (dotProductBilin ℝ ℝ r))
  have hd_map' : HasSum (fun k : ℕ => dotProduct (Matrix.vecMul p0 (A ^ k)) r) (dotProduct d r) := by
...
theorem policy_gradient_theorem_of_local_series
  ...
  have hJ_deriv :
      HasDerivAt J (dotProduct d' r) theta := by
    have hdot :
  ...
  have hrow_formula :
      HasDerivAt (fun t => Matrix.vecMul (d t) (1 - Pi t))
        (Matrix.vecMul d' (1 - Pi theta) - Matrix.vecMul (d theta) Pi') theta := by
    simpa [vecMulCLM, sub_eq_add_neg, Matrix.vecMul_add, Matrix.vecMul_one, Matrix.vecMul_neg,
  ...
  have hrow_eq0 :
      (0 : ι → ℝ) = Matrix.vecMul d' (1 - Pi theta) - Matrix.vecMul (d theta) Pi' :=
  ...
  have hfinal :
      dotProduct d' r = dotProduct (Matrix.vecMul (d theta) Pi') (q theta) := by
    calc
      dotProduct d' r
          = dotProduct d' (Matrix.mulVec (1 - Pi theta) (q theta)) := by rw [hcol_theta]
      _ = dotProduct (Matrix.vecMul d' (1 - Pi theta)) (q theta) := by
        simpa using Matrix.dotProduct_mulVec d' (1 - Pi theta) (q theta)
      _ = dotProduct (Matrix.vecMul (d theta) Pi') (q theta) := by rw [hrow_eq]
